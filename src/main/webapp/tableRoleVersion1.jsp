...
<body>
<form action="/role/findByCondition" method="post">
    <div class="row">
...
<nav aria-label="Page navigation">
    <ul class="pagination">
        <li id="numHead">
            <a href="${sessionScope.kv.URI}?page=${sessionScope.kv.page-1<1?1:sessionScope.kv.page-1}" aria-label="Previous">
                <span aria-hidden="true">&laquo;</span>
            </a>
        </li>
        <c:forEach var="i" step="1" begin="1" end="${sessionScope.kv.totalPage}">
            <c:if test="${i == sessionScope.kv.page}">
                <li class="active"><a href="${sessionScope.kv.URI}?page=${i}">${i}<span class="sr-only">(current)</span></a></li>
            </c:if>
            <c:if test="${i != sessionScope.kv.page}">
                <li><a href="${sessionScope.kv.URI}?page=${i}">${i}</a></li>
            </c:if>
        </c:forEach>
        <li id="numEnd">
            <a href="${sessionScope.kv.URI}?page=${sessionScope.kv.page+1>sessionScope.kv.totalPage?sessionScope.kv.page:sessionScope.kv.page+1}"
               aria-label="Next">
